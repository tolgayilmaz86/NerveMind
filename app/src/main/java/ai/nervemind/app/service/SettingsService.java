...
public class SettingsService implements SettingsServiceInterface {
     ...
     *
     * @param settingRepository the repository for setting persistence
     * @param encryptionService the service for encrypting sensitive settings
     ...
     * Loads default settings and populates the cache.
     */
    @PostConstruct
    public void init() {
        initializeDefaults();
    ...
    public String exportAsJson() {
        try {
            ...
            throw new DataParsingException("Failed to export settings", "settings", e);
        }
    }
    ...
    public void importFromJson(String json) {
        try {
            ...
            for (Map.Entry<String, String> entry : settings.entrySet()) {
                importSingleSetting(entry.getKey(), entry.getValue());
            }
            log.info("Imported {} settings", settings.size());
        } catch (JsonProcessingException e) {
            throw new DataParsingException("Failed to import settings", "settings", e);
        }
    }

    private void importSingleSetting(String key, String value) {
        try {
            setValue(key, value);
        } catch (IllegalArgumentException _) {
            log.warn("Skipping unknown setting during import: {}", key);
        }
    }
    ...
    private void validateValue(SettingEntity entity, String value) {
        ...
        try {
            ...
            validateNumberRange(entity.getKey(), value, validation);
            validateEnumOptions(entity.getKey(), value, validation);
        } catch (JsonProcessingException _) {
            log.warn("Invalid validation rules for setting: {}", entity.getKey());
        }
    }

    private void validateNumberRange(String key, String value, Map<String, Object> validation) {
        if (!validation.containsKey("min") && !validation.containsKey("max")) {
            return;
        }

        double numValue = Double.parseDouble(value);
        validateMinimum(key, numValue, validation);
        validateMaximum(key, numValue, validation);
    }

    private void validateMinimum(String key, double numValue, Map<String, Object> validation) {
        ...
        if (numValue < min) {
            throw new IllegalArgumentException(
                    String.format("Value for '%s' must be at least %s", key, min));
        }
    }

    private void validateMaximum(String key, double numValue, Map<String, Object> validation) {
        ...
        if (numValue > max) {
            throw new IllegalArgumentException(
                    String.format("Value for '%s' must be at most %s", key, max));
        }
    }

    private void validateEnumOptions(String key, String value, Map<String, Object> validation) {
        ...
        if (!options.contains(value)) {
            throw new IllegalArgumentException(
                    String.format("Invalid value for '%s'. Must be one of: %s", key, options));
        }
    }
