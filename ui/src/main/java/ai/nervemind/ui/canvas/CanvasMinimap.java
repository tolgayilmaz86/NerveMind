...
public class CanvasMinimap extends StackPane {
    ...
    /**
     * Creates a new CanvasMinimap for the given canvas.
     *
     * @param canvas The workflow canvas to map
     */
    public CanvasMinimap(WorkflowCanvas canvas) {
        ...
        // Background with programmatic colors (Rectangle doesn't support CSS fill)
        ...
        // Viewport indicator rectangle with programmatic colors
    ...
    private void setupInteraction() {
        ...
        viewportRect.setOnMouseDragged(e -> {
            if (isDragging) {
                ...
                // Clamp to map bounds
                newX = Math.clamp(newX, 0, mapPane.getWidth() - viewportRect.getWidth());
                newY = Math.clamp(newY, 0, mapPane.getHeight() - viewportRect.getHeight());
    ...
    private void updateNodeDots() {
        ...
        for (NodeView node : canvas.getNodeViews()) {
            ...
            // Set color programmatically (Rectangle doesn't support CSS fill)
    ...
    public void updateViewport() {
        ...
        // Clamp to map bounds
        vpX = Math.clamp(vpX, PADDING, mapWidth - vpWidth + PADDING);
        vpY = Math.clamp(vpY, PADDING, mapHeight - vpHeight + PADDING);
