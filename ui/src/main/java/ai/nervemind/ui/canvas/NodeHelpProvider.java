...
public final class NodeHelpProvider {
   ...
   * @return help content for the node type, or null if not found
   */
  public static NodeHelp getHelp(String nodeType) {
    return switch (nodeType) {
      ...
      case "code" -> new NodeHelp(
          "Code",
          "Execute custom JavaScript or Python code to transform data.",
          """
              ...
              Python Modes (Settings → Python):
              • Embedded (GraalPy): Zero-install, sandboxed
              • External: Full pip ecosystem
              • Auto: External if available, else embedded
              """,
          """
              ═══ JAVASCRIPT EXAMPLE ═══
              ...
              return {'word_frequency': dict(freq)}
              """);
   ...
   *
   * @param title               The display title for the help
   * @param shortDescription    A brief summary of the node's purpose
   * @param detailedDescription Comprehensive documentation about the node
   * @param sampleCode          Example JSON/code for usage
   */
